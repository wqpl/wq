...
impl Resolver {
    ...
    fn resolve_node(&mut self, node: AstNode) -> AstNode {
        match node {
            ...
            AstNode::Postfix { object, items, explicit_call } => {
                ...
                if explicit_call || self.should_call(&object) {
                    if let AstNode::Variable(name) = *object.clone() {
                        ...
                        AstNode::CallAnonymous { object, args: items }
                    }
                } else {
            ...
            AstNode::BinaryOp { left, operator, right } => AstNode::BinaryOp {
            ...
            AstNode::IndexAssign { object, index, value } => AstNode::IndexAssign {
                object: Box::new(self.resolve_node(*object)),
                index: Box::new(self.resolve_node(*index)),
            ...
            AstNode::Return(expr) => {
                AstNode::Return(expr.map(|e| Box::new(self.resolve_node(*e))))
            }
