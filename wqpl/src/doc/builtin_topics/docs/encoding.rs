...
pub(super) const DECODE: BuiltinDoc = BuiltinDoc {
    ...
    details: "`decode[bytes;codec]` converts byte-shaped data into a string. `bytes` may be one int or bigint in `0..=255`, a list of ints, or a list of ints/bigints in that range. `codec` is an encoding label understood by `encoding_rs`, such as `\"utf-8\"` or `\"windows-1252\"`. The default mode is `\"s\"` for strict, which errors on invalid input; mode `\"r\"` replaces invalid byte sequences.",
...
pub(super) const VALID_BYTES: BuiltinDoc = BuiltinDoc {
    ...
    details: "`bytes?[x]` checks whether `x` can be converted to a byte vector: a single int or bigint in `0..=255`, a list whose items are all byte-sized ints, or a list of ints/bigints whose items are all bytes. It does not decode or inspect text; it only checks the numeric byte shape accepted by `encode`, `decode`, and `fwrite`.",
