...
pub(super) const DECODE: BuiltinDoc = BuiltinDoc {
    ...
    details: "`decode[bytes;codec]` converts byte-shaped data into a string. `bytes` may be one int from `0` through `255` or a list of ints in that range. `codec` is an encoding label understood by `encoding_rs`, such as `\"utf-8\"` or `\"windows-1252\"`. The default mode is `\"s\"` for strict, which errors on invalid input; mode `\"r\"` replaces invalid byte sequences.",
...
pub(super) const ENCODE: BuiltinDoc = BuiltinDoc {
    ...
    details: "`encode[value;codec]` converts a string, char, or unit to a list of byte values. `codec` is an encoding label understood by `encoding_rs`. The default mode is `\"s\"` for strict, which errors when the value cannot be represented in the target encoding; mode `\"r\"` uses replacement behavior.",
...
pub(super) const VALID_BYTES: BuiltinDoc = BuiltinDoc {
    ...
    details: "`bytes?[x]` checks whether `x` is one int from `0` through `255` or a list whose items are all ints in that range. It does not decode or inspect strings; it only checks the byte shape accepted by `encode`, `decode`, and `fwrite`.",
