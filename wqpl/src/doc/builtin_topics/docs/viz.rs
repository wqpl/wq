...
pub(super) const SHOWTABLE: BuiltinDoc = BuiltinDoc {
    ...
    details: concat!(
        "`showtable[table;opts]` accepts a dict of atom values, a list of dicts, a dict of lists, ",
        ...
        "written to stdout. Strings render as text cells; other lists in dict-of-lists inputs",
        "expand as columns. Numeric columns are right-aligned using display width, ",
        "so wide Unicode cells line up with narrow cells. Options are `cols`, `limit`, ",
