...
fn contains_op(value: &Value, needle: &str) -> bool {
    ...
    fn numeric_erfc_zero() {
        let result =
            simplify_cas_value(&Value::from_cas_call("erfc", vec![Value::Int(0)])).unwrap();
    ...
    fn numeric_gamma_five() {
        // gamma(5) = 4! = 24
        let result =
            simplify_cas_value(&Value::from_cas_call("gamma", vec![Value::Int(5)])).unwrap();
    ...
    fn numeric_heaviside_pos() {
        let result =
            simplify_cas_value(&Value::from_cas_call("heaviside", vec![Value::float(3.0)]))
                .unwrap();
        assert_eq!(result, Value::float(1.0));
    }
    ...
    fn diff_integrate_roundtrip_inverse_x_sqrt_is_clean() {
        ...
        .unwrap();
        let integrand =
            cas_div(Value::Int(1), cas_mul(vec![x.clone(), sqrt_part]).unwrap()).unwrap();
