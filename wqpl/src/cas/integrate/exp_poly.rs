//! Integration of polynomial * exponential: int P(x)*e^(k*x) dx via
//! undetermined coefficients.
//!
//! The ansatz int P(x)*e^(k*x) dx = e^(k*x) * R(x)  with deg(R) = deg(P)
...
///   rn = pn/k
///   r_{n-1} = (p_{n-1} - n*rn)/k
///   ...
///   r_j = (p_j - (j+1)*r_{j+1})/k
