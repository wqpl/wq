...
impl Formatter {
    ...
    ///
    /// Pipeline: lex → parser (with CST enabled) → CST root → Doc IR →
    /// width-aware render. The AST is parsed alongside as a witness for
    /// the CST construction's correctness; it is not consumed here.
