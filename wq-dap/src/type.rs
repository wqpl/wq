...
pub struct ColumnDescriptor {
    ...
    /// specified. Values: 'string', 'number', 'bool', 'unixTimestampUTC'
...
pub struct Source {
    ...
    /// Since a `sourceReference` is only valid for a session, it can not be
...
pub struct SourceBreakpoint {
    ...
    /// corresponding capability `supportsHitConditionalBreakpoints` is
    /// true.
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// request. Specfication: [BreakpointLocation](https://microsoft.github.io/debug-adapter-protocol/specification#Types_BreakpointLocation)
#[derive(Deserialize, Serialize, Debug, Default, Clone)]
#[serde(rename_all = "camelCase")]
pub struct BreakpointLocation {
    ...
    /// UTF-16 code units and the client capability `columnsStartAt1`
...
pub struct CompletionItem {
    ...
    /// request) where the completion text is added. The position is
    /// measured in UTF-16 code units and the client capability
    /// `columnsStartAt1` determines whether it is 0- or 1-based. If the
    /// start position is omitted the text is added at the
    /// location specified by the `column` attribute of the `completions`
    /// request.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct DisassembledInstruction {
    ...
    /// but can be omitted afterwards if this instruction maps to the same
    /// source file as the previous instruction.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub enum VariablePresentationHintKind {
    ...
    /// synthetic object introduced by the adapter for rendering purposes, e.g.
    /// an index range for large arrays.
    Virtual,
...
pub struct VariablePresentationHint {
    ...
    /// 'hasObjectId': Indicates that the object can have an Object ID created
    /// for it.
    /// 'canHaveObjectId': Indicates that the object has an Object ID associated
    /// with it.
    /// 'hasSideEffects': Indicates that the evaluation had side effects.
    /// 'hasDataBreakpoint': Indicates that the object has its value tracked by
    /// a data breakpoint.
    /// etc.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// expensive and/or produce side-effects. A typical example are
    /// properties based on a getter function.
    /// Please note that in addition to the `lazy` flag, the variable's
    /// `variablesReference` is expected to refer to a variable that will
    /// provide the value through another `variable` request.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct Scope {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub line: Option<i64>,
    /// Start position of the range covered by the scope. It is measured in
    /// UTF-16 code units and the client capability `columnsStartAt1`
    /// determines whether it is 0- or 1-based.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub column: Option<i64>,
    ...
    /// code units and the client capability `columnsStartAt1` determines
...
pub struct StackFrame {
    ...
    /// client.
    pub line: i64,
    /// Start position of the range covered by the stack frame. It is measured
    /// in UTF-16 code units and the client capability `columnsStartAt1`
    /// determines whether it is 0- or 1-based. If attribute `source` is
    /// missing or doesn't exist, `column` is 0 and should be ignored by the
    /// client.
    pub column: i64,
    ...
    /// the `restart` request and the corresponding capability
    ...
    /// A value of `label` can be used to indicate that the frame is an
    /// artificial frame that is used as a visual label or separator. A
    /// value of `subtle` can be used to change the appearance of a frame in
    /// a 'subtle' way. Values: 'normal', 'label', 'subtle'
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct Variable {
    ...
    /// This can be a multi-line text, e.g. for a function the body of a
    /// function. For structured variables (which do not have a simple
    /// value), it is recommended to provide a one-line representation of
    /// the structured object. This helps to identify the structured object
    ...
    /// `variables` request as long as execution remains suspended. See [Lifetime of Object References](https://microsoft.github.io/debug-adapter-protocol/overview#lifetime-of-objects-references)
    /// in the Overview section of the specification for details.
    pub variables_reference: i64,
...
pub struct Message {
    ...
    /// that have the requirement that every user visible error message
    /// needs a corresponding error i64, so that users or customer support
    /// can find information about the specific error more easily.
    pub id: i64,
