...
pub struct InvalidatedEventBody {
    ...
    /// missing, empty, or if values are not understood, the client should
...
pub struct OutputEventBody {
    ...
    /// value is a hint, clients might ignore the hint and assume the
    /// `console` category.
    ...
    /// messages. Values:
    /// 'start': Start a new group in expanded mode. Subsequent output events
    /// are members of the group and should be shown indented.
    /// The `output` attribute becomes the name of the group and is not
    /// indented. 'startCollapsed': Start a new group in collapsed mode.
    /// Subsequent output events are members of the group and should be
    /// shown indented (as soon as the group is expanded).
    /// The `output` attribute becomes the name of the group and is not
    /// indented. 'end': End the current group and decrease the indentation
    /// of subsequent output events.
...
pub enum Event {
    ...
    /// before the initialize request has finished).
    ///
    ...
    /// Since the capabilities are dependent on the client and its UI, it might
    ...
    /// expected to make a ‘best effort’ in honouring individual
    /// capabilities but there are no guarantees. Only changed capabilities
    /// need to be included, all other capabilities keep their values.
    ///
    ...
    /// launch or continue. It is only necessary to send a continued event
    ...
    /// requires that the client needs to re-render the data snapshot
    /// previously requested. Debug adapters do not have to emit this event
    /// for runtime changes like stopped or thread events because in that
    /// case the client refetches the new state anyway. But the event can be
    /// used for example to refresh the UI after rendering formatting has
    /// changed in the debug adapter.
    ///
    ...
    /// only be sent if the corresponding capability supportsMemoryEvent is
    /// true. Clients typically react to the event by re-issuing a
    /// readMemory request if they show the memory identified by the
    /// memoryReference and if the updated memory range overlaps the
    /// displayed range. Clients should not make assumptions how individual
    /// memory references relate to each other, so they should not assume
    /// that they are part of a single continuous address range and might
    /// overlap.
    ///
    ...
    /// The event signals that the progress reporting needs to be updated with a
    /// new message and/or percentage.
    /// The client does not have to update the UI immediately, but the clients
    /// needs to keep track of the message and/or percentage values.
    ...
    /// to some condition. This can be caused by a breakpoint previously
    /// set, a stepping request has completed, by executing a debugger
    /// statement etc.
    ///
