...
/// Represents a response message that is either a cancellation or a short error
/// string.
...
pub enum ResponseMessage {
    ...
    /// [`Response::success`](Response::success) is false. This raw error
    /// might be interpreted by the client and is not shown in the UI.
    #[serde(untagged)]
    Error(String),
}
...
pub struct DataBreakpointInfoResponse {
    ...
    /// available. If a `variablesReference` or `frameId` is passed, the
    /// `dataId` is valid in the current suspended state, otherwise it's
    /// valid indefinitely. See 'Lifetime of Object References' in the
    /// Overview section for details. Breakpoints set using the `dataId` in
    /// the `setDataBreakpoints` request may outlive the lifetime of the
    /// associated `dataId`.
...
pub struct SetDataBreakpointsResponse {
    /// Information about the breakpoints.
    /// The array elements are in the same order as the elements of the
    /// `breakpoints` array in the arguments.
    pub breakpoints: Vec<Breakpoint>,
}
...
pub enum ResponseBody {
    ...
    ///
    /// NOTE: we are straying away from the spec here, as the spec says that the
    /// response body is optional, but we are always returning a body
    /// because I could not find a way to express skipping the optional body
    /// with serde (and serializing null will make the schema validation
    /// complain).
    ///
    ...
    Restart,
    /// Response to `restartFrame` request. This is just an acknowledgement, so
    ...
    RestartFrame,
    /// Response to `reverseContinue` request. This is just an acknowledgement,
    ...
    /// This includes the actual code location and whether the breakpoint could
    /// be verified. The breakpoints returned are in the same order as the
    /// elements of the breakpoints (or the deprecated lines) array in the
    /// arguments.
    ///
    ...
    /// When a data breakpoint is hit, a `stopped` event (with reason `date
    ...
    /// about each exception breakpoint or filter. The Breakpoint objects
    /// are in the same order as the elements of the `filters`,
    /// `filterOptions`, `exceptionOptions` arrays given as arguments. If both
    /// `filters` and `filterOptions` are given, the returned array must
    /// start with filters information first, followed by `filterOptions`
    /// information.
    ///
    /// The `verified` property of a `Breakpoint` object signals whether the
    /// exception breakpoint or filter could be successfully created and
    /// whether the condition or hit count expressions are valid. In case of
    /// an error the message property explains the problem. The id property can
    /// be used to introduce a unique ID for the exception breakpoint or
    /// filter so that it can be updated subsequently by sending breakpoint
    /// events.
    ///
    /// For backward compatibility both the `breakpoints` array and the
    /// enclosing body are optional. If these elements are missing a client
    /// is not able to show problems for individual exception breakpoints or
    /// filters.
    ///
    /// NOTE: we are straying away from the spec here, as the spec says that the
    /// response body is optional, but we are always returning a body
    ...
    Terminate,
    /// Response to `terminateThreads` request. This is just an acknowledgement,
    /// so no body field is required.
    ///
