...
pub struct Version {
    ...
    /// Minor version.
    pub minor: i64,
    /// Patch version. Historically, this is "x" in the changelog for most
    /// versions. That value is represented as `None` here.
    pub patch: Option<i64>,
    /// The git commit in the DAP repo that corresponds to this version.
    /// Please note that historically, versions (as of 1.62.x) are not tagged in
    /// the DAP repo. Until that changes, we are using the commit that
    /// updates the JSON-schema in the gh-pages branch.
...
///
/// Please note that historically, the DAP changelog hasn't been super accurate
/// and the versions (as of 1.62.x) are not tagged in the DAP repo. Until that
/// changes, we are using the commit that *adds the corresponding JSON-schema in
/// the **gh-pages** branch*.
