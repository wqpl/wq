...
pub trait Renderer {
    ...
    fn compute_layout(
        ...
        continuation_prompt_size: Option<Position>,
        default_prompt: bool,
        line: &LineBuffer,
        info: Option<&str>,
    ) -> Layout {
        let continuation = continuation_prompt_size.unwrap_or_default();
        ...
        if let Some(info) = info {
            end = self.calculate_position(info, end, Position::default());
        }
    ...
    /// After each newline, the next line starts at `continuation` position.
