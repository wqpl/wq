...
pub struct State<'out, 'prompt, H: Helper, P: Prompt + ?Sized> {
    ...
    continuation_prompt_size: Position,
}
...
impl<'out, 'prompt, H: Helper, P: Prompt + ?Sized> State<'out, 'prompt, H, P> {
    pub fn new(
        ...
        let prompt_size =
            out.calculate_position(prompt.raw(), Position::default(), Position::default());
        ...
        Self {
            ...
            continuation_prompt_size: Position::default(),
        }
    }
    ...
    pub fn is_default_prompt(&self) -> bool {
        self.continuation_prompt.is_none() && self.layout.default_prompt
    }
    ...
    pub fn validate(&mut self) -> Result<ValidationResult> {
        if let Some(validator) = self.helper {
            ...
            match result {
                ValidationResult::Incomplete(ref prompt) => {
                    if let Some(p) = prompt {
                        self.continuation_prompt = Some(p.clone());
                        self.continuation_prompt_size = self.out.calculate_position(
                            p,
                            Position::default(),
                            Position::default(),
                        );
...
impl<H: Helper, P: Prompt + ?Sized> Refresher for State<'_, '_, H, P> {
    ...
    fn refresh_prompt_and_line(&mut self, prompt: &str) -> Result<()> {
        let prompt_size =
            self.out
                .calculate_position(prompt, Position::default(), Position::default());
        self.hint();
        self.highlight_char(CmdKind::Other);
