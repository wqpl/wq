...
impl Renderer for PosixRenderer {
    ...
    fn refresh_line<P: Prompt + ?Sized>(
        ...
        if let Some(highlighter) = highlighter {
            ...
            let mut parts = highlighted.split('\n');
            if let Some(first) = parts.next() {
                self.buffer.push_str(first);
            }
...
mod test {
    ...
    fn prompt_with_ansi_escape_codes() {
        ...
        let pos = out.calculate_position(
            "\x1b[1;32m>>\x1b[0m ",
            Position::default(),
            Position::default(),
        );
    ...
    fn test_line_wrap() {
        ...
        let old_layout = out.compute_layout(prompt_size, None, default_prompt, &line, None);
        ...
        let new_layout = out.compute_layout(prompt_size, None, default_prompt, &line, None);
