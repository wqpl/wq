        ...
        match validator.as_ref() {
            ...
            Some(v) => {
                ...
                match v.validate(&mut ctx)? {
                    ...
                    validate::ValidationResult::Incomplete(ref prompt) => {
...
impl<H: Helper, I: History> Editor<H, I> {
    ...
    fn readline_with<P: Prompt + ?Sized>(
        ...
        if self.term.is_unsupported() {
            debug!(target: "wq_rl", "unsupported terminal");
            ...
            debug!(target: "wq_rl", "stdin is not a tty");
    ...
    fn readline_edit<P: Prompt + ?Sized>(
        ...
        loop {
            ...
            if cmd == Cmd::Complete && s.helper.is_some() && self.config.completion_enabled() {
    ...
    /// ```
    /// let mut rl = wq_rl::DefaultEditor::new()?;
